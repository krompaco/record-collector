...
public class ContentController : Controller
{
    ...
    public IResult Report()
    {
        ...
        this.LogTime();
        return Results.Content(sb.ToString(), "text/plain", Encoding.UTF8);
    }

    [HttpGet]
    public IResult Properties()
    {
        var model = this.GetContentProperties();

        var json = Results.Json(model);
        this.LogTime();
        return json;
    }
...
#pragma warning restore IDE0007 // Use implicit type
            ...
            && path.Contains('.', StringComparison.Ordinal))
        {
            ...
            this.LogTime();
            return Results.File(foundFullName, contentType ?? "text/plain");
        }
    ...
    private IResult GetXmlActionResultAndLogTime(ListPage listPage, LayoutViewModel viewModel, string siteUrl, bool isCategory)
    {
        var rssUrl = new Uri(
            new Uri(siteUrl),
        ...
        var result = rssXmlBuilder.BuildResult();
